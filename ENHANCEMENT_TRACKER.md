...
## Round 29 — Playbook V02: P2 Alembic + P4 Cloud Run/CD + P5.S5 k6

Closed the remaining infra/ops phases from the reviewer doc:

- **P2.S1–S4 — Alembic migrations**: `backend/alembic.ini`, async-aware
  `alembic/env.py` (sync driver for migrations, DATABASE_URL override),
  `script.py.mako`, autogenerated `versions/*_initial_schema.py` (all 9 tables:
  incidents, remediations, audit, notifications, refresh_tokens, error_budgets,
  outbox, burn_acks, audit_log). `Database.init_schema()` runs `alembic upgrade
  head` with a `create_all` fallback; in-memory SQLite uses create_all (Alembic
  can't share that connection). RepositoryBundle.init() now calls init_schema().
  **+3 tests**; verified upgrade head builds all tables.
- **P4.S3 — Dockerfile hardening**: non-root UID 1001, HEALTHCHECK, /data owned
  by appuser, alembic+redis installed.
- **P4.S1 — scripts/deploy.sh**: build+push+`gcloud run deploy` with Secret
  Manager `--set-secrets` (DT_API_TOKEN, GOOGLE_API_KEY, PAGERDUTY_ROUTING_KEY,
  OPSGENIE_API_KEY, JWT_SECRET).
- **P4.S1 — cloudrun.yaml**: Secret Manager `secretKeyRef` bindings (no plaintext
  secrets), startup/liveness probes, min/maxScale.
- **P4.S2 — .github/workflows/deploy.yml**: WIF auth + deploy.sh on main/tags.
- **P5.S5 — k6/matchday_load.js**: ramping-VU matchday profile, thresholds
  health p95<50ms / slo p95<200ms / errors<1%.

Backend **494 pass** (+3); frontend tsc+build green; 7 node RBAC tests.

### PHASE STATUS vs reviewer doc (84→96 target)
- P1 CI + Playwright — DONE
- P2 Alembic — DONE
- P3 Redis idempotency — DONE (pre-existing)
- P4 Cloud Run hardening/CD/deploy.sh/cloudrun secrets — DONE
- P5.S1 DT multi-window metrics — DONE (pre-existing)
- P5.S2 alert routing — DONE · P5.S3 dispatch — DONE · P5.S4 Davis — DONE
- P5.S5 k6 load — DONE
- P6.M1–M9 width module backends — DONE (all nine)
- REMAINING: frontend pages for the new P6 modules; SQL-backing the width-module
