...
async def compose_digest(ctx) -> str:
    """Build a human-readable burn/suppression digest from current state."""
    alerts = await ctx.burn_alerts()  # cross-venue (system digest)
    page = sum(1 for a in alerts if getattr(a.severity, "value", a.severity) == "page")
    ticket = sum(1 for a in alerts if getattr(a.severity, "value", a.severity) == "ticket")
...
class DigestScheduler:
    ...
    def __init__(self, ctx, interval_seconds: float,
                 dispatch: Callable[[str], Awaitable[None]] | None = None) -> None:
        self._ctx = ctx
    ...
    async def _run(self) -> None:
        while True:
            try:
                ...
                msg = await compose_digest(self._ctx)
